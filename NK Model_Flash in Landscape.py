...
class NK_model(object):
    ...
    def repeat_random_walk(self):
        ...
        self.walk_genotype = [i[0] for i in self.walk_list]
        
        if len(self.walk_list)>6:
            self.walk_genotype = self.walk_genotype[:6]
            self.walk_list = self.walk_list[:6]
    ...
    @staticmethod
    def multiply_certain_element(list1, index_list):
        '''Multiply the elements in list1 according to the indexes stored in index_list
        parameters: 
        ----------
            list1: list
    ...
    def calculate_e_effect(self):
        ...
        for i in self.pairwise_w:
            ...
            multipy_one_mutant_fitness = self.multiply_certain_element(self.w, corr_one_mutant_index) # get the multiply results
                                                                        # (i.e., W1*W2, W1*W3,...)
            self.pairwise_e_effect.append(i-multipy_one_mutant_fitness) # calculate e and store in self.pairwise_e_effect
        
        
    
    def multiple_random_walk(self, times):
        '''Allow multiple random walk in the landscape (times: how many self.pairwise_e_effect we would like to generate
        and append into self.multiple_pairwise_e_effect)'''
        
        self.multiple_random_walk = []
        
        self.multiple_pairwise_e_effect_rw = []
        self.multiple_landscape_subset_rw = []
        
        
        while len(self.multiple_pairwise_e_effect_rw)<times:

            self.repeat_random_walk()  # do the random walk
            self.search_different_allele()  # get the changed alleles in an random walk
            
            if len(self.diff_allele_indice) == 5: # this means that 5 sites have been changed in the random walk (only
                        # under this condition append the self.pairwise_e_effect into self.multiple_pairwise_e_effect)
                
                self.initialize()
                
                self.multiple_random_walk.append(self.walk_list)
                self.multiple_landscape_subset_rw.append(self.landscape_subset)
                self.multiple_pairwise_e_effect_rw.append(self.pairwise_e_effect)
            
            self.start_type = self.generate_start_type(self.n) # generate the start_type again
            
            
    def multiple_adaptive_walk(self, times):
        '''Allow multiple adaptive walk in the landscape (times: how many self.pairwise_e_effect we would like to generate
        and append into self.multiple_pairwise_e_effect)'''
        
        self.multiple_adaptive_walk = [] 
       
        self.multiple_pairwise_e_effect_aw = []
        self.multiple_landscape_subset_aw = []
        
        
        while len(self.multiple_pairwise_e_effect_aw)<times:

            self.repeat_adaptive_walk() # do the adaptive walk
            self.search_different_allele() # get the changed alleles in an adaptive walk
            
            if len(self.diff_allele_indice) == 5:  # this means that 5 sites have been changed in the adaptive walk (only
                        # under this condition append the self.pairwise_e_effect into self.multiple_pairwise_e_effect)
                
                self.initialize()
                
                self.multiple_adaptive_walk.append(self.new_walk_list)
                
                self.multiple_landscape_subset_aw.append(self.landscape_subset)
                self.multiple_pairwise_e_effect_aw.append(self.pairwise_e_effect)
    ...
    def initialize(self):
        ...
        self.creat_landscape_subset()  # creat the landscape subset based on the adaptive walk
        self.get_one_mutant()
        self.construct_pairwise_index()
        self.get_pairwise_genotype()
        self.calculate_e_effect()
