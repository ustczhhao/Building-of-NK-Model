...
class NK_model(object):
    ...
    def get_epistic_site(self):
        '''Generate the epistic networks among the sites (k/2 flanking neighbors)'''

        self.epistic_site = []
        all_site_indice = list(range(self.n))
        print(all_site_indice)
        
        for i in range(self.n):

            if i > self.k /2 and self.n-i > self.k/2:
                epistic_indice = all_site_indice[int(i-self.k/2):i]+ all_site_indice[i+1:int(i+self.k/2)+1]
            elif i>self.k/2 and self.n-i <= self.k/2:
                epistic_indice = all_site_indice[int(i-self.k/2):i]+ all_site_indice[i+1:]+ \
                all_site_indice[0:int(i+1+ self.k/2- self.n)]
            else :
                epistic_indice = all_site_indice[i+1:int(i+self.k/2)+1]+ all_site_indice[0:i]+ \
                all_site_indice[int(self.n-(self.k/2 - i)):]
            
            epistic_indice.sort()
#             print(epistic_indice)
            self.epistic_site.append(epistic_indice)
